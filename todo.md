...
- [ ] Initialize Node.js/Express project and install dependencies (`express`, `sequelize`, `mysql2`, etc.).
- [ ] Set up the database connection using Sequelize (configured for Aiven SSL).
- [ ] Define Models and Associations: `User`, `Category`, `Product`, `CartItem`, `Order`, `OrderItem`.
- [ ] Create the central error handling middleware and standard response utilities.

**Sprint 2: Authentication & Authorization**
- [ ] Implement user registration (`POST /api/auth/register`) with bcrypt hashing.
- [ ] Implement login (`POST /api/auth/login`) generating an `accessToken` and a httpOnly cookie for `refreshToken`.
- [ ] Implement token refresh (`POST /api/auth/refresh`) and logout (`POST /api/auth/logout`).
- [ ] Create `verifyToken` and `isAdmin` middleware to protect routes.

**Sprint 3: Catalog (Products & Categories)**
- [ ] Create CRUD endpoints for Categories (Admin protected: POST, PUT, DELETE; Public: GET).
- [ ] Create CRUD endpoints for Products (Admin protected: POST, PUT, DELETE; Public: GET with pagination/filters).

**Sprint 4: Shopping Cart**
- [ ] Implement add to cart (`POST /api/cart/items`) with stock validation.
- [ ] Implement update item quantity (`PUT /api/cart/items/:productId`).
- [ ] Implement remove item (`DELETE /api/cart/items/:productId`) and clear cart (`DELETE /api/cart`).
- [ ] Implement fetch cart (`GET /api/cart`).

**Sprint 5: Orders & User Management**
- [ ] Implement checkout process (`POST /api/orders/checkout`) turning cart items into an order, running within a database transaction to decrement stock atomically.
- [ ] Implement order history for customers (`GET /api/orders/my`).
- [ ] Implement order management for admins (listing all orders, advancing order states like pending -> processing -> shipped).
- [ ] Implement user profiles and admin user list/deactivation.
- [ ] Generate Swagger UI documentation at `/api-docs`.
- [ ] **Push to Repository:** *Once the backend is fully complete and tested locally, it is time to push the code to your `shopapi-backend` GitHub repository.*
